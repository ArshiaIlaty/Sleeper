...
| `stage_dispersion.py` | Within-stage **dispersion** (the *non-avg* view): recomputes the per-epoch band powers / entropy / resp-rate / spindle values the mean CSVs collapsed and reports their SD/CV/percentiles instead. Reuses the exact primitives of `eeg_spectral`/`nk_features`, so a recomputed mean matches the mean CSVs (verified to 5e-5) |
| `export_dispersion_features.py` | CLI: stream the physio EDFs → a wide per-recording **dispersion feature** CSV (the *non-avg* companion). Merges with the other wide CSVs on `(bids_folder, session)`; `--no-complexity` skips the O(n²) entropy pass, `--shard`/`--nshards` parallelise the large cohort |
| `epoch_features.py` | The **long** (fully un-aggregated) per-epoch primitives: EEG band power + ratios per epoch, capped-subsample per-epoch entropy, and one record per detected spindle (reuses `eeg_spectral`/`nk_features` internals) |
| `export_epoch_features.py` | CLI: stream the physio EDFs → the **long per-epoch tables** in `exports/per_epoch/` (`epoch_eeg_<cohort>.csv` one row per recording×stage×epoch; `spindles_<cohort>.csv` one row per spindle). For EDA and sequence/temporal models |
| `hrv_windows.py` | **Non-avg per-stage HRV**: HRV is undefined per 30 s epoch, so the un-aggregated grain is a short (~120 s) window of beats *within* a stage. `hrv_windows_by_stage` yields full linear HRV per (stage, window); `hrv_dispersion` summarises their spread. Reuses `nk_features`' exact R-peak detection + `_hrv_from_rr`, so a window's HRV matches the pooled per-stage definition |
| `export_hrv_windows.py` | CLI: stream the physio EDFs → the **non-avg HRV tables** in `exports/hrv/` (`hrv_windows_<cohort>.csv` LONG, one row per recording×stage×window; `hrv_dispersion_<cohort>.csv` WIDE, per-stage SD/CV/pXX of the windowed HRV). One ECG pass; `--no-freq` skips the frequency domain; `--shard`/`--nshards` parallelise |
| `signal_quality.py` | **NeuroKit2 signal-quality** assessment + diagnostic plots: `ecg_quality` (averageQRS per-sample score + zhao2018 verdict, overall & per-stage), `rsp_quality`, an EEG/EOG **windowed** flat-line/clip/NaN sanity check (SD-vs-active-level, so coarse-ADC quantization isn't mistaken for dropout), and `ecg_plot`/`rsp_plot` PNG renderers |
| `export_quality.py` | CLI: stream the physio EDFs → a wide per-recording **signal-quality** CSV in `exports/quality/`, and (with `--plots`) one `nk.ecg_plot` + one `nk.rsp_plot` PNG per recording under `exports/quality/plots/<cohort>/` |
| `EXPORTS_MANIFEST.md` | **Catalog of every feature CSV** — the wide/long/quality shapes, column families, join keys, which script writes each, and how to regenerate them |
