...
| `signal_quality.py` | **NeuroKit2 signal-quality** assessment + diagnostic plots: `ecg_quality` (averageQRS per-sample score + zhao2018 verdict, overall & per-stage), `rsp_quality`, an EEG/EOG flat-line/clip/NaN sanity check, and `ecg_plot`/`rsp_plot` PNG renderers |
