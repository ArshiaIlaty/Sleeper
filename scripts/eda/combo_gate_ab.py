#!/usr/bin/env python3
"""3-ARM GATE for the full advanced-feature combo (Group-3 dynamics + heavy EEG).

Settles, honestly, the user's hypothesis: does adding the multi-channel EEG
network features (B1 connectivity + B2 microstates) ON TOP OF the Group-3 dynamics
(D1/D2/E1) beat the champion under strict LOSO — even though the dynamics subset
ALONE did not (dyn_gate_ab -> NO-SHIP, ΔAC-AUROC -0.028)?

Three LOSO arms on the existing feature_matrix_local_plus.npz champion X, joining
the two advanced-feature CSVs by bids_folder:

  baseline        : champion columns only               (previous submission)
  base+dyn        : champion + dyn (D1/D2/E1)            (previous experiment)
  base+dyn+net    : champion + dyn + net (B1/B2)         (the new full combo)

Same pooled fitter + official metric path as dyn_gate_ab / site_arch_ab. The
optional --filter applies the roadmap's per-fold univariate AUROC>min-auc gate to
the ADDED columns only (dyn__/net__), TRAIN-ONLY (no leakage). Paired bootstrap
CIs on the aligned LOSO out-of-fold predictions for the two contrasts that matter:
(full − baseline) and (full − base+dyn). Verdict SHIP only if the full combo beats
the CHAMPION on AC-AUROC (lower CI > 0, or >=0 with reward strictly up). Aggregate
stats only.

Usage (on pdmle, as arshia_ilaty_physio26):
  PYTHONPATH=/data-temp/physio-viewer/bench/repo python3 combo_gate_ab.py \
    ...
    --net-csv /data-temp/physio-viewer/exports/net_features_standard.csv \
...
NON_FEATURE = {
    ...
    # bookkeeping columns emitted by the exporters (never features):
...
def main():
    ...
    ap.add_argument("--filter", action="store_true",
                    help="per-fold univariate AUROC>min-auc filter on added (dyn__/net__) cols")
    ...
    dyn_rows, dyn_cols = load_csv(args.dyn_csv)
    net_rows, net_cols = load_csv(args.net_csv)
    D, dmiss = join(pids, dyn_rows, dyn_cols)
    N, nmiss = join(pids, net_rows, net_cols)
    dyn_names = ["dyn__" + c for c in dyn_cols]
    net_names = ["net__" + c for c in net_cols]

    print(f"cache X={X.shape} y+={int(y.sum())}/{len(y)} "
          f"sites={dict(zip(*np.unique(sites, return_counts=True)))}", flush=True)
    print(f"dyn cols={len(dyn_cols)} ({dmiss} missing) | "
          f"net cols={len(net_cols)} ({nmiss} missing)", flush=True)
    print(f"filter={'ON' if args.filter else 'off'} min_auc={args.min_auc}\n", flush=True)

    X_base = X
    X_dyn = np.concatenate([X, D], axis=1)
    X_full = np.concatenate([X, D, N], axis=1)
    names_base = list(names)
    names_dyn = list(names) + dyn_names
    names_full = list(names) + dyn_names + net_names
    thr = float(y.mean())
    ADDED = ("dyn__", "net__")
    ...
    yt_d, yp_d, ya_d = loso(X_dyn, names_dyn, args.filter)
    ...
    report("base+dyn", yt_d, yp_d, ya_d)
    report("base+dyn+net", yt_f, yp_f, ya_f)

    assert np.array_equal(yt_b, yt_f) and np.array_equal(yt_b, yt_d), \
    ...
    for label, hi, lo in [("full − baseline", yp_f, yp_b),
                          ("full − base+dyn", yp_f, yp_d),
                          ("base+dyn − baseline", yp_d, yp_b)]:
    ...
    print(f"\nVERDICT: {'SHIP — full combo beats champion under LOSO' if ship else 'DO NOT SHIP — no LOSO improvement over champion'}",
          flush=True)
    print("COMBO_GATE_SHIP" if ship else "COMBO_GATE_NOSHIP", flush=True)
