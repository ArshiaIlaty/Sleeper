...
import sig_features  # noqa: E402  (vendored NeuroKit2/spectral/CAP feature package)

# Track 2 (full-feature) submission. DEFAULT_PRESET is submit_full: the arch
# submission core PLUS the vendored signal-feature block (per-stage HRV, EEG
# spectral/spindles, oxygenation, resp events, SO-spindle coupling, RSWA, CAP).
# This is the within-distribution champion (438-feat 80/20 AC-AUROC ~0.83); it
# hedges Track 1 (LOSO-optimised) for the same-3-sites hidden-test case.
DEFAULT_PRESET = "submit_full"
EXTRACT_CACHE_PRESET = "extract_all"
REWARD_THRESHOLD_MODE = "site_decade"
KAISER_FINETUNE = True
# Single pooled feature head for Track 2 — no separate Kaiser alt preset (the alt
# head is Track 1's tuned autonomic stack; a 42-feat alt would clash with the
# full 300+ feature vector and adds nothing to the within-distribution model).
KAISER_ALT_PRESET = None
...
def extract_signal_features(phys_data, phys_fs, algo_data, algo_fs):
    """Full vendored signal-feature block (Track 2): per-stage NeuroKit2 HRV,
    EEG spectral/spindles, oxygenation, respiratory events, SO-spindle coupling,
    REM-atonia/RSWA and CAP — 336 features in a fixed order. Reproduces the
    offline export recipe (scripts/viewer/export_{nk,report,micro}_features.py)
    at inference. Any missing stream/failed value -> NaN; never raises."""
    try:
        return sig_features.signal_feature_row(
            phys_data or {}, phys_fs or {}, algo_data or {}, algo_fs)
    except Exception:
        names = list(sig_features.SIGNAL_FEATURE_COLUMNS)
        return _safe([], len(names)), names
...
def extract_all_features(record, data_folder, csv_path=DEFAULT_CSV_PATH, preset=DEFAULT_PRESET):
    ...
    # Physio EDF is loaded at most ONCE and shared by the autonomic + signal
    # blocks (the ~170-460 MB waveform decode is the expensive step). The signal
    # block also needs it kept alive until after CAISR is loaded, so we hold
    # phys/phys_fs across the CAISR section and delete them at the very end.
    need_phys = cfg.get("include_autonomic") or cfg.get("include_signal")
    phys, phys_fs = None, None
    if need_phys:
    ...
    if cfg.get("include_autonomic"):
        try:
            a_feat, a_names = extract_autonomic_features(
                phys if phys else {}, phys_fs if phys_fs else {}, csv_path)
        except Exception:
            a_feat, a_names = extract_autonomic_features({}, {}, csv_path)
        blocks_feat.append(a_feat)
        blocks_name.extend(a_names)

    if cfg.get("include_caisr") or cfg.get("include_arch") or cfg.get("include_signal"):
        ...
        if cfg.get("include_signal"):
            # Full vendored signal block: needs BOTH the physio waveforms and the
            # CAISR annotation streams (stage/arousal/resp).
            s_feat, s_names = extract_signal_features(phys, phys_fs, algo, algo_fs)
            blocks_feat.append(s_feat)
            blocks_name.extend(s_names)
